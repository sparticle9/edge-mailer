...
describe('Email', () => {
  ...
  describe('getEmailData', () => {
    ...
    it('should include CC headers and keep BCC envelope-only', () => {
      const email = new Email({
        from: 'sender@example.com',
        to: 'recipient@example.com',
      ...
      expect(email.bcc).toEqual([{ email: 'bcc@example.com' }])
    ...
    it('should not override custom standard headers', () => {
      ...
      expect(data).not.toContain('BCC: custom-bcc@example.com')
